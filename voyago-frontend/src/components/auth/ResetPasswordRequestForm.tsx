...
export default function ResetPasswordRequestForm() {
  ...
  const sendRequest = async (
    ...
    try {
      const savedRequestResponse = await fetch(`/api/auth/forgot-password`, {
        method: "POST",
        headers: {
    ...
    >
      {({
        ...
        <div className="w-4/12 border-1 border-gray-200 rounded-2xl flex justify-center items-center bg-gray-50 p-8">
          ...
          >
            <FormHeader headerH1="Enter your email" />
            <fieldset>
              <input
            ...
            <button
              ...
              onClick={() =>
                values.email ? setRequest(true) : setRequest(false)
              }
            >
              Send
            </button>
            {request && (
              <p className="text-black text-center mt-2">
                If a user with this email exists, you will receive an email
                shortly!
              </p>
            )}
          </form>
